        ...
        <HorizontalScroller>
          <div className="flex gap-4 sm:gap-5 snap-x snap-mandatory mt-12 pb-4">
            {products.map((product) => (
              ...
              >
                <Link href={`/product/${product.slug || product.id}`} className="absolute inset-0 z-20" aria-label={`View ${product.name}`}></Link>
                ...
                >
                  {product.images?.[0] ? (
                    <Image src={product.images[0]} alt={`${product.name} - Organic Ayurvedic Skincare | Pureable`} fill className="w-full h-full object-cover" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" />
                  ) : null}
                  {/* Quick View Overlay */}
